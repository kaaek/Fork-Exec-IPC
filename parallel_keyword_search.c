...
int main(int numberOfArguments, char* argumentVector[]) {
    ...
    printf("[Parent] Creating %d child processes...", numberOfChildren); 
    ...
    waitForChildren(childrenPIDs, numberOfChildren, filenames); // the Children can't execute this because they exit(...) from searchFile(...)
}

int waitForChildren(pid_t *childrenPIDs, int numberOfChildren, char **filenames){
    ...
    printf("Total files searched: %d", numberOfChildren);
}

void searchFile(char *filename, char *keyword) {
    printf("[Child PID: %d] Searching in %s...", getpid(), filename);

    int occurrences = countKeywordOccurrences(filename, keyword);

    printf("[Child PID: %d] Found %d Searching in %s", getpid(), occurrences, filename);
...
int countKeywordOccurrences(char *filename, char *keyword){
    ...
    if (filePointer == NULL) {
        fprintf(stderr, "[Child PID: %d] Error opening file %s", getpid(), filename);
        exit(EXIT_FAILURE);
    }
